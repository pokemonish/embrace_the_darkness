...
// Тут пиздец какой костыль, просто беги и не оглядывайся.
define([], function(){
var Runner = null;
...
Random.prototype = {
  ...
  nextRanged: function(min, max) {
    return min + (this.next() - 1) % (max - min + 1);
  }
}
 ...
 * @export
 */
Runner = function(outerContainerId, playable, seed, onPlayerDied) {
...
Runner.prototype = {
  ...
  init: function() {
    ...
    this.horizon = new Horizon(this.canvas, this.spriteDef, this.dimensions,
        this.config.GAP_COEFFICIENT, this.random);

    // Distance meter
    this.distanceMeter = new DistanceMeter(this.canvas,
          this.spriteDef.TEXT_SPRITE, this.dimensions.WIDTH, this.random);

    // Draw t-rex
    this.tRex = new Trex(this.canvas, this.spriteDef.TREX, this.random);
  ...
  playIntro: function() {
    if (!this.started && !this.crashed) {
      ...
      // this.containerEl.style.webkitAnimation = 'intro 5.8s ease-out 1 both';
  ...
  update: function() {
    ...
    } while (deltaTime > 0.0001 && deltaTime < 20 - (this.timeDebt || 0))
    this.time = now;

    if(deltaTime > 0.0001) {
      this.timeDebt =  (deltaTime - 20) + (this.timeDebt || 0);
      // console.log(this.timeDebt)
    }
   ...
   * @param {Event} e
   */
  onKeyDown: function(e, canRunGame) {
  ...
  start: function() {
    if (!this.activated && !this.crashed) {
      ...
      setTimeout(function() {
        that.loadSounds();
        that.activated = true;
        that.playSound(that.soundFx.BUTTON_PRESS);
        that.tRex.startJump(that.currentSpeed);
      }, 0);
    }
  },
  ...
  destroy: function() {
    ...
    clearInterval(this.gameInterval_);
    clearInterval(this.resizeTimerId_);
 ...
 * @param {number}
 */
function getRandomNum(min, max, random) {
  if(random) {
    return random.nextRanged(min, max);
  } else {
 ...
 * @constructor
 */
function GameOverPanel(canvas, textImgPos, restartImgPos, dimensions, random) {
  this.canvas = canvas;
  ...
  this.random = random || (function(){ throw 'random' })();
 ...
 * @param {number} speed
 */
function Obstacle(canvasCtx, type, spriteImgPos, dimensions, gapCoefficient, speed, random) {
  ...
  this.random = random || (function(){ throw 'random' })();
  this.size = getRandomNum(1, Obstacle.MAX_OBSTACLE_LENGTH, this.random);
...
Obstacle.prototype = {
  ...
  init: function(speed) {
    ...
    if (Array.isArray(this.typeConfig.yPos))  {
      ...
      this.yPos = yPosConfig[getRandomNum(0, yPosConfig.length - 1, this.random)];
    } else {
  ...
  getGap: function(gapCoefficient, speed) {
    ...
    return getRandomNum(minGap, maxGap, this.random);
  },
 ...
 * @constructor
 */
function Trex(canvas, spritePos, random) {
  this.canvas = canvas;
  this.canvasCtx = canvas.getContext('2d');
 ...
 * @constructor
 */
function DistanceMeter(canvas, spritePos, canvasWidth, random) {
  this.canvas = canvas;
  this.canvasCtx = canvas.getContext('2d');
 ...
 */
function Cloud(canvas, spritePos, containerWidth, random) {
  this.canvas = canvas;
  this.canvasCtx = this.canvas.getContext('2d');
  this.spritePos = spritePos;
 ...
 * @constructor
 */
function HorizonLine(canvas, spritePos, random) {
  this.spritePos = spritePos;
  this.canvas = canvas;
  this.canvasCtx = canvas.getContext('2d');
 ...
 * @constructor
 */
function Horizon(canvas, spritePos, dimensions, gapCoefficient, random) {
  this.canvas = canvas;
  this.canvasCtx = this.canvas.getContext('2d');
...
Horizon.prototype = {
  ...
  init: function() {
    this.addCloud();
    this.horizonLine = new HorizonLine(this.canvas, this.spritePos.HORIZON, this.random);
  },
  ...
  updateObstacles: function(deltaTime, currentSpeed) {
    ...
    var obstacleSpawnTime = this.obstacleSpawnTime || getTimeStamp();
    var currentTime = getTimeStamp();

    if(currentTime >= obstacleSpawnTime) {
      this.addNewObstacle(currentSpeed);
      this.obstacleSpawnTime = obstacleSpawnTime + getRandomNum(600, 1300, this.random);
    }
  ...
  addNewObstacle: function(currentSpeed) {
    var obstacleTypeIndex = getRandomNum(0, Obstacle.types.length - 1, this.random);
    ...
    if (this.duplicateObstacleCheck(obstacleType.type) ||
      ...
      this.obstacles.push(new Obstacle(this.canvasCtx, obstacleType,
          ...
          this.gapCoefficient, currentSpeed, this.random));
  ...
  addCloud: function() {
    this.clouds.push(new Cloud(this.canvas, this.spritePos.CLOUD,
        this.dimensions.WIDTH, this.random));
  }
};
})();
